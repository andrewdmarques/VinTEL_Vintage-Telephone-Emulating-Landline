...
layout = [
    ...
    [sg.Radio('Dial', 'RADIO1', key='dial'), sg.Radio('Hook', 'RADIO1', key='hook'), sg.Radio('Audio', 'RADIO1', key='audio'), sg.Radio('Coins', 'RADIO1', key='coins')],
...
while True:
    ...
    if event == 'Add Interaction':
        if not all([values['from'], values['to'], any([values['dial'], values['hook'], values['audio'], values['coins']])]):
