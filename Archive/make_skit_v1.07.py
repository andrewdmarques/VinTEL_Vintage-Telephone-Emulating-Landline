...
# Description
##########################################################################
# make_skit.py (v1.06) Helps organize the skit file based on the input files.
...
layout = [
    ...
    [sg.Table(values=[],
              headings=['Current File', 'Input Type', 'Input Value', 'Next File', 'First File'],
...
def create_csv_file(file_path, input_data):
    with open(file_path, 'a', newline='') as csvfile:
        ...
        csvwriter.writerow(['Current File', 'Input Type', 'Input Value', 'Next File', 'First File'])  # Write header row
...
while True:
    ...
    if event == 'Add Interaction':
        ...
        new_input = [values['from'], input_type, input_value, values['to'], 'yes' if values['first_file'] else 'no']
