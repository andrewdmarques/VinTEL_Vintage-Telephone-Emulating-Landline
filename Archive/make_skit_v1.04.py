...
# Description
##########################################################################
# make_skit.py (v1.04) Helps organize the skit file based on the input files.
...
def get_user_input(values):
    if values['dial']:
        return 'dial-' + sg.popup_get_text('Enter the numerical value:')
    elif values['hook']:
        return 'hook-on'
    elif values['voice']:
        return 'voice-' + sg.popup_get_text('Enter the keywords:')
    else:
        return ''
...
while True:
    ...
    if event == 'Add Another Input':
        ...
        type_value = get_user_input(values)
        new_input = [values['from'], values['input'], type_value, values['to'], 'yes' if values['first_file'] else 'no']
